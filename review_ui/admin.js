...
async function loadSummary() {
  try {
    ...
    byId("analyzableTotal").textContent = payload.analyzable_sessions;
    ...
    byId("conditionRows").innerHTML = Object.entries(payload.conditions).map(([condition, values]) => `
      <tr><td>${labels[condition] || condition}</td><td>${values.responses}</td><td>${formatRate(values.initial_accuracy)}</td><td>${formatRate(values.final_accuracy)}</td><td>${formatRate(values.correct_ai_acceptance)}</td><td>${formatRate(values.wrong_ai_rejection)}</td></tr>`).join("");
